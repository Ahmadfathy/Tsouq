...
module.exports = (options = {enternce_direction: 'left',exit_direction: 'left',speed: '',animation: 'slide'}) => {
	var side_nav = {
		_animation: {
			speed: toLower(options.speed),
			enternce: `${toLower(options.animation)}In${($('html').attr('dir') === 'ltr') ? capitalize(options.enternce_direction) : capitalize(options.exit_direction)} ${options.speed}`,
			exit: `${toLower(options.animation)}Out${($('html').attr('dir') === 'ltr') ? capitalize(options.exit_direction) : capitalize(options.enternce_direction)} ${options.speed}`,
		},
		_init: function () {
			this._cach_dom();
			this._bind_events();
		...
		_bind_events: function () {
			var _this = this; // refare to side_nav
