	...
	return gulp
			...
			.pipe(pug({
				...
				data: {
					...
					lang: "en",
					sitename: {en: "Tsuoq.com", ar: "تسوق.كوم"}
				}}
			))
...
gulp.task('js', function () {
	return browserify({ entries: config.assets_dir + '/' + config.js.file_path_and_pattern, debug: true }) // src/scripts/script.js
		.transform('babelify', { presets: ['env'] })
		.bundle()
		.pipe(source('app.js'))
		.pipe(buffer())
		.pipe(config.is_prod ? uglify({ mangle: false, output: { beautify: false } }) : util.noop()) // uglify only in is_prod
...
gulp.task('images', function () {
	return gulp
			...
			.pipe(cache(imagemin([
				...
				imageminJpegRecompress({ loops: 6, min: 40, max: 85, quality: 'low' }),
...
// ─── 4- WATCH TASKS ─────────────────────────────────────────────────────────────
//
gulp.task('dev', gulp.series('fonts', 'images', 'sass', 'rtl', 'js', 'pug', function () {
